...
| A8 | `mock-agent.sh` ships in the Release bundle | None: it is sealed by the signature and only enabled by `VIBE_ENABLE_MOCK_AGENT`, which only the user can set | **Kept**: it is what lets the smoke test run against the notarized build. |
